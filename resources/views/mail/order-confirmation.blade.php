...
<body
    ...
    <div
        ...
        {{-- Header --}}
        <div
            style="background:linear-gradient(135deg, #0f0c2e 0%, #1e1047 50%, #3b1264 100%); padding:48px 40px 40px; text-align:center; position:relative;">
            <div
                style="display:inline-block; background:rgba(255,255,255,0.12); border:1px solid rgba(255,255,255,0.2); border-radius:50px; padding:6px 18px; margin-bottom:20px;">
                <span
                    style="color:#c4b5fd; font-size:11px; font-weight:600; letter-spacing:2px; text-transform:uppercase;">{{ $t['badge'] }}</span>
            </div>
            ...
            <p style="color:#b8aff5; font-size:15px; margin:0 0 24px; line-height:1.6;">
                {{ $t['hi'] }} <strong
                    style="color:#ede9fe;">{{ $order->customer_first_name }}</strong>{{ $t['greeting'] }}
            </p>
        ...
        <div style="padding:36px 40px;">
            ...
            <div style="margin-bottom:28px;">
                ...
                <div style="background:#1a1735; border:1px solid #2d2956; border-radius:14px; padding:18px 20px;">
                    <p style="color:#ede9fe; font-weight:600; font-size:16px; margin:0 0 5px;">
                        {{ $order->customer_first_name }} {{ $order->customer_last_name }}</p>
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:28px;">
                <div style="background:#1a1735; border:1px solid #2d2956; border-radius:14px; padding:18px 20px;">
                    <p
                        style="font-size:10px; font-weight:700; color:#4e4a80; letter-spacing:1.5px; text-transform:uppercase; margin:0 0 10px;">
                        📄 &nbsp;{{ $t['billing_address'] }}</p>
                    <address style="font-style:normal; color:#9490cc; font-size:14px; line-height:1.7;">
                        @foreach ($order->billing_address ?? [] as $key => $value)
                            @if ($value)
                                {{ $key === 'country' ? CountryService::getLabel($value) : $value }}<br>
                            @endif
                        @endforeach
                    </address>
                </div>
                    ...
                    <table style="width:100%; border-collapse:collapse; font-size:14px;">
                        ...
                        <tbody>
                            @foreach ($order->items as $item)
                                <tr style="border-bottom:1px solid #2d2956;">
                                    <td style="padding:13px 16px; color:#ede9fe; font-weight:500;">
                                        {{ $item->product_title }}</td>
                                    <td style="padding:13px 16px; text-align:center; color:#9490cc;">
                                        {{ $item->quantity }}</td>
                                    <td style="padding:13px 16px; text-align:right; color:#9490cc;">
                                        @if ($item->sale_price)
                                            <span
                                                style="text-decoration:line-through; color:#4e4a80; margin-right:4px;">€{{ $price($item->unit_price) }}</span>
                                            <span
                                                style="color:#34d399; font-weight:600;">€{{ $price($item->sale_price) }}</span>
                                        @else
                                            €{{ $price($item->unit_price) }}
                                        @endif
                                    </td>
                                    <td style="padding:13px 16px; text-align:right; color:#ede9fe; font-weight:600;">
                        ...
                        <tfoot>
                            <tr style="background:linear-gradient(135deg, #1e1b4b, #3b1264);">
                                <td colspan="3"
                                    style="padding:16px; text-align:right; color:#c4b5fd; font-weight:600; font-size:13px; letter-spacing:0.5px;">
                                    {{ $t['order_total'] }}</td>
                                <td
                                    style="padding:16px; text-align:right; color:#ffffff; font-weight:700; font-size:18px;">
            ...
            @if ($order->notes)
                <div
                    style="background:#1c1304; border:1px solid #78350f; border-radius:12px; padding:16px 20px; margin-bottom:28px;">
                    <p
                        style="font-size:10px; font-weight:700; color:#d97706; letter-spacing:1.5px; text-transform:uppercase; margin:0 0 8px;">
                        📝 &nbsp;{{ $t['order_notes'] }}</p>
