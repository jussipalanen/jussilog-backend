...
<body style="margin:0; padding:30px 16px; background:linear-gradient(135deg, #080714 0%, #130d2e 100%); min-height:100vh;">
    <div style="max-width:620px; margin:0 auto; background:#100e25; border-radius:20px; overflow:hidden; border:1px solid #2a2754; box-shadow:0 24px 64px rgba(0,0,0,0.6);">
        ...
        <div style="background:linear-gradient(135deg, #0f0c2e 0%, #1e1047 50%, #3b1264 100%); padding:48px 40px 40px; text-align:center;">
            ...
            <h1 style="color:#ffffff; font-size:26px; font-weight:700; margin:0 0 10px; line-height:1.3;">{{ $t['email_title'] }}</h1>
            ...
            @if ($invoice->invoice_number)
                <div style="display:inline-block; background:rgba(255,255,255,0.08); border:1px solid rgba(196,181,253,0.25); border-radius:10px; padding:12px 28px;">
                    <span style="color:#c4b5fd; font-size:11px; font-weight:600; letter-spacing:1px; text-transform:uppercase; display:block; margin-bottom:4px;">{{ $t['invoice_number'] }}</span>
                    <span style="color:#ffffff; font-size:20px; font-weight:700; font-family:monospace;">{{ $invoice->invoice_number }}</span>
                </div>
            @endif
        </div>
        ...
        <div style="padding:36px 40px;">
            ...
            <div style="margin-bottom:28px;">
                <p style="font-size:10px; font-weight:700; color:#4e4a80; letter-spacing:1.5px; text-transform:uppercase; margin:0 0 12px;">{{ $t['bill_to'] }}</p>
                <div style="background:#1a1735; border:1px solid #2d2956; border-radius:14px; padding:18px 20px;">
                    <p style="color:#ede9fe; font-weight:600; font-size:16px; margin:0 0 5px;">{{ $invoice->customer_first_name }} {{ $invoice->customer_last_name }}</p>
                    @if ($invoice->customer_email)
                        <p style="color:#9490cc; font-size:14px; margin:0 0 3px;">✉️ &nbsp;{{ $invoice->customer_email }}</p>
                    @endif
            ...
            <div style="margin-bottom:28px;">
                <p style="font-size:10px; font-weight:700; color:#4e4a80; letter-spacing:1.5px; text-transform:uppercase; margin:0 0 12px;">{{ $t['address'] }}</p>
                <div style="background:#1a1735; border:1px solid #2d2956; border-radius:14px; padding:18px 20px; color:#9490cc; font-size:14px; line-height:1.8;">
            ...
            <div style="margin-bottom:28px;">
                <p style="font-size:10px; font-weight:700; color:#4e4a80; letter-spacing:1.5px; text-transform:uppercase; margin:0 0 12px;">{{ $t['items'] }}</p>
                <div style="border:1px solid #2d2956; border-radius:14px; overflow:hidden;">
                    <table style="width:100%; border-collapse:collapse; font-size:13px;">
                        <thead>
                            <tr style="background:#1e1b4b;">
                                <th style="padding:11px 14px; text-align:left; color:#c4b5fd; font-weight:600; font-size:11px; letter-spacing:0.5px;">{{ $t['description'] }}</th>
                                <th style="padding:11px 14px; text-align:center; color:#c4b5fd; font-weight:600; font-size:11px;">{{ $t['qty'] }}</th>
                                <th style="padding:11px 14px; text-align:right; color:#c4b5fd; font-weight:600; font-size:11px;">{{ $t['unit_price'] }}</th>
                                <th style="padding:11px 14px; text-align:right; color:#c4b5fd; font-weight:600; font-size:11px;">{{ $t['tax'] }}</th>
                                <th style="padding:11px 14px; text-align:right; color:#c4b5fd; font-weight:600; font-size:11px;">{{ $t['total'] }}</th>
                            </tr>
                        </thead>
                        <tbody>
            ...
            {{-- Summary --}}
            <div style="margin-bottom:28px;">
                <p style="font-size:10px; font-weight:700; color:#4e4a80; letter-spacing:1.5px; text-transform:uppercase; margin:0 0 12px;">{{ $t['summary'] }}</p>
                <div style="background:#1a1735; border:1px solid #2d2956; border-radius:14px; padding:18px 20px;">
            ...
            @if ($invoice->notes)
            <div style="margin-bottom:28px;">
                <p style="font-size:10px; font-weight:700; color:#4e4a80; letter-spacing:1.5px; text-transform:uppercase; margin:0 0 12px;">{{ $t['notes'] }}</p>
                <div style="background:#1a1735; border:1px solid #2d2956; border-radius:14px; padding:18px 20px;">
